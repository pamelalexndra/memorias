...
fun ErrorState(message: String, onRetry: () -> Unit) {
  Column(
    ...
    TextButton(onClick = onRetry) {
      Text(text = "Intentar de nuevo", color = Siena)
    }
  }
}
...
fun EmptyDayCard() {
  ...
  )
  {
    ...
    Spacer(modifier = Modifier.height(8.dp))
    Text(
      text = "Cuando tengas recuerdos con esta fecha, aparecerán aquí.",
      color = BrownMid,
      textAlign = TextAlign.Center,
      fontSize = 13.sp
    )
  ...
  )
  {
    ...
    Text(
      text = if (pendingDeleteCount > 0) {
        "Tienes $pendingDeleteCount foto${
          if (pendingDeleteCount != 1) "s"
          else ""
        } pendiente${
          if (pendingDeleteCount != 1) "s"
          else ""
        } de revisar antes de borrar."
      } else {
    ...
    if (deletedCount > 0) {
      Spacer(modifier = Modifier.height(8.dp))
      Text(
        text = "Ya borraste $deletedCount foto${if (deletedCount != 1) "s" else ""} · %.1f MB liberados".format(
          freedMb
        ), color = BrownMid, fontSize = 12.sp, textAlign = TextAlign.Center
      )
    }
    Spacer(modifier = Modifier.height(18.dp))
    if (pendingDeleteCount > 0) {
      TextButton(onClick = onReviewDelete) {
        Text(
          text = "Revisar fotos a borrar",
          color = SwipeDelete
        )
      }
    }
    TextButton(onClick = onRestart) {
      Text(text = "Volver a revisar desde el inicio", color = Siena)
    }
  }
}